...
def customer_create_order(reference_id, customer_id, delivery_address, order_products, initiator_id):
    # TODO: Validation
    logger.info("Create Order Attempt: by %s", initiator_id)
    try:
        if order_exists(reference_id):
            logger.error(
                "Create Order Attempt Failed: by %s | Order Already Exists", initiator_id)
            create_audit_log("Create Order", user_id=initiator_id,
        ...
        if not is_valid:
            if total == "Product Does Not Exist":
                logger.error(
                    "Create Order Attempt Failed: by %s | %s", initiator_id, total)
                create_audit_log(
                    "Create Order", user_id=initiator_id, details=total)
        ...
        if not user_exists(customer_id):
            logger.error(
                "Create Order Attempt Failed: by %s | Customer Does Not Exist", initiator_id)
            create_audit_log("Create Order", user_id=initiator_id,
        ...
        try:
            order = insert_order(reference_id, customer_id,
        ...
        logger.info("Create Order Attempt Successful: by %s | Created %s",
                    initiator_id, order.order_id)
        create_audit_log("Create Order", user_id=initiator_id,
                         details=f"Created {order.order_id}")
        return {"success": True}

    except DatabaseQueryError as e:
        logger.error("Create Order Attempt Failed: by %s | %s",
                     initiator_id, e)
        create_audit_log("Create Order", user_id=initiator_id,
                         details=e.message)
        raise
...
def customer_get_orders(search, sort_by, sort_order, page, limit, initiator_id):
    ...
    try:
        ...
        logger.error("Get Orders Attempt Failed: by %s | %s", initiator_id, e)
        create_audit_log("Get Orders", user_id=initiator_id,
...
def get_order_details(order_id, initiator_id):
    ...
    try:
        try:
            ...
            if len(details) <= 0:
                logger.error(
                    "Get Order Details Attempt Failed: by %s | No Order Details Found", initiator_id)
                create_audit_log(
                    "Get Order Details", user_id=initiator_id, details="No Order Details Found")
            ...
            logger.info(
                "Get Order Details Attempt Successful: by %s", initiator_id)
            create_audit_log("Get Order Details",
                             initiator_id, details="Success")
        ...
        logger.error(
            "Get Order Details Attempt Failed: by %s | %s", initiator_id, e)
...
def insert_order(reference_id, customer_id, delivery_address, total):
    order = Order(order_uuid=reference_id, customer_id=customer_id,
                  status="Pending", delivery_address=delivery_address, total=total)
